...
import { Send, ArrowLeft, Bot, Sparkles, Zap, Brain, MessageCircle, Star } from 'lucide-react';
...
export function ChatbotPage() {
  ...
  const [currentScene, setCurrentScene] = useState<'pyramid' | 'atom' | 'dna' | 'solar-system'>('atom');
  ...
  const getSceneFromMessage = (text: string): 'pyramid' | 'atom' | 'dna' | 'solar-system' => {
    const lowerText = text.toLowerCase();
    if (lowerText.includes('pyramid') || lowerText.includes('egypt') || lowerText.includes('ancient')) {
      return 'pyramid';
    } else if (lowerText.includes('dna') || lowerText.includes('biology') || lowerText.includes('genetic')) {
      return 'dna';
    } else if (lowerText.includes('solar') || lowerText.includes('planet') || lowerText.includes('space')) {
      return 'solar-system';
    }
    return 'atom';
  };
  ...
  const suggestions = [
    ...
    { text: "Tell me about Egyptian pyramids", icon: <Star className="w-4 h-4" /> },
    { text: "How does DNA work?", icon: <Brain className="w-4 h-4" /> },
    { text: "Show me the solar system", icon: <Sparkles className="w-4 h-4" /> }
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0A0F1C] via-[#0D1B2A] to-[#1B263B] relative overflow-hidden">
      <div className="absolute inset-0 opacity-30">
        ...
        <div className="absolute bottom-20 right-20 w-48 h-48 bg-[#778DA9] rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      <div className="relative z-50 ">
        {!isActive ? (
          <div className={`h-[120vh] flex items-center justify-center p-8 transition-all duration-800 ${isTransitioning ? 'idle-to-active' : ''}`}>
            <div className="text-center max-w-4xl mx-auto">
              <div className="relative mt-28">
                ...
                <p className="text-xl text-[#778DA9] mb-12 leading-relaxed max-w-2xl mx-auto font-light">
                  Experience the future of learning with our AI-powered assistant. 
                  Get instant answers with interactive 3D visualizations.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 max-w-4xl mx-auto">
                {suggestions.map((suggestion, index) => (
                  ...
                  >
                    ...
                    <p className="text-[#E0E1DD] font-semibold text-lg mb-2">{suggestion.text}</p>
                    <p className="text-[#778DA9] text-sm">Interactive 3D visualization included</p>
                  </button>
                ))}
              </div>
          ...
          <div className="flex transition-all duration-500 h-[100vh]">
            <div className={`flex flex-col transition-all duration-500 ${showPreview ? 'flex-1' : 'w-full'}`}>
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                {messages.map((message, index) => (
                  <div
                    key={message.id}
                    className={`flex ${message.isUser ? 'justify-end' : 'justify-start'} message-enter message-enter-active`}
                  ...
                  >
                    <div className="flex items-start space-x-4 max-w-[80%]">
                      ...
                      <div
                        className={`p-5 rounded-3xl backdrop-blur-sm transition-all duration-300 hover:scale-[1.01] ${
                          message.isUser
                            ? 'bg-gradient-to-br from-[#415A77] to-[#778DA9] text-[#E0E1DD] rounded-br-lg shadow-xl'
                            : 'bg-[#1B263B]/60 text-[#E0E1DD] border border-[#415A77]/30 rounded-bl-lg shadow-xl'
                        }`}
                      >
                        <p className="text-sm leading-relaxed font-medium">{message.text}</p>
                        <p className="text-xs text-[#778DA9] mt-3 font-medium">
                      ...
                      {message.isUser && (
                        <div className="w-10 h-10 bg-gradient-to-br from-[#778DA9] to-[#E0E1DD] rounded-2xl flex items-center justify-center flex-shrink-0">
                          <span className="text-[#0D1B2A] text-sm font-bold">U</span>
                        </div>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div className="bg-[#1B263B]/60 backdrop-blur-sm border border-[#415A77]/30 p-5 rounded-3xl rounded-bl-lg shadow-xl">
                        <div className="flex space-x-2 items-center">
                          ...
                          <div className="w-2 h-2 bg-[#778DA9] rounded-full typing-dots" />
                          <div className="w-2 h-2 bg-[#778DA9] rounded-full typing-dots" />
                          <span className="text-[#778DA9] text-sm ml-2 font-medium">AI is thinking...</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="h-full flex flex-col">
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-xl font-bold text-[#E0E1DD]">3D Visualization</h3>
                  ...
                  <div className="flex-1 relative rounded-2xl overflow-hidden border border-[#415A77]/30 bg-[#0D1B2A]/40">
                    ...
                    <div className="absolute top-4 right-4 bg-[#1B263B]/80 backdrop-blur-sm px-3 py-1 rounded-full">
                      <span className="text-xs text-[#778DA9] font-medium">Live Preview</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 p-4 bg-[#0D1B2A]/40 rounded-2xl border border-[#415A77]/20">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex space-x-1">
                        ...
                        <div className="w-2 h-2 bg-[#415A77] rounded-full animate-pulse" style={{ animationDelay: '0.5s' }} />
                      </div>
                    </div>
                  </div>
                </div>
